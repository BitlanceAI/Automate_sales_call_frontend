...
import AutomateSalesCallTool from "./index";
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const updatedHistory = [{ number: phoneNumber, time: new Date().toLocaleString() }, ...callHistory.slice(0, 4)];
  ...
  >
    <i className="fa fa-play" /> Call Now
                            ...
                            <div className="mt-6">
                              ...
                              <ul className="text-sm text-muted-foreground list-disc list-inside">
                                {callHistory.map((entry, idx) => (
                                  <li key={idx}>{entry.number} <span className="text-xs text-gray-500 ml-2">{entry.time}</span></li>
                                ))}
                              </ul>
                            </div>
                          )}
                        ...
                        <div className="mt-8 max-w-md mx-auto text-center text-muted-foreground text-sm">
                          <p>
                            ⚠️ <strong>Important:</strong> You can only call numbers that are verified in your
                            Twilio trial account. To remove this limitation, consider upgrading your Twilio plan.
                          </p>
                        </div>
